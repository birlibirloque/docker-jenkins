...
main() {
    ...
    # If the (single) argument is a file, assume this file contains the list of plugins
    # and store the plugin list the from file without linebreaks to the plugins variable
    if [[ ($# -eq 1) && (-f $1) ]]; then
        plugins=$(cat "$1" | tr '\n' ' ')
    fi
